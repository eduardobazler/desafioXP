...
Aqui você vai encontrar os detalhes de como executar-lo localmente, as decições tomadas para a resolução e detalhes sobre as rotas.
...
5. Após a certificação da existencia do banco de dados, rode o comando abaixo para fazer as migrações necessárias.
  * `npx sequelize db:migrate`

6. Para popular o banco, inicialmente, rode o comando abaixo.
  * `npx sequelize db:seed:all`

7. Após a certificação, dos passos acima, rode o comando abaixo para inicar a aplicação. 🚀
  * `npm start`


8. Para o ambiente de desenvolvimento, existe a opção abaixo, que ira verificar as alterações do código e recarrega-lo atumotaticamente.
...
# Descrição das tomadas de decições e abordagens do problema

  Modelo do banco de dados: 

  ![DB](imagens/banco_de_dados.png)

  A decição dessa modulução, consiste nas seguintes ideias:

  * Cada usuario tem suas informações de acesso em uma tabela única, tabela Users, separando a responsabilidade das entidades que lidam com a conta das que lidam com o as informações de acesso.


  * Cada usuário tem uma Contas única que se relaciona de forma única com a a tabela de Users, criando uma relação de 1:1 entre a tebala Users e Contas, onde se contra o seu saldo bancário.


  * A tabela Acoes contém as informações das ações, o código único da ação, empresa, tag e valor.


  * A tabela Conta_acoes contém a relação da Conta com o Ação, suportando a relção de N:N entre a tabela Acoes com a tebla Contas, trazendo a quantidade dessa relação.


  * A tabela Corretora contém a quantidade de ações que a corretora possui, criando uma relção de 1:1.

  * A tabela Historio guarda as informações da compra ou da venda de uma ação, registrando o tipo, venda ou compra, o código da ação,
  o código da conta, a quantidade comprada ou vendida e data que isso ocorreu.

  * A tabela extrato se refere as transações de saques e depósitos na conta do usuário, guardando as informações do código da conta,
  valor da tranferencia, o tipo, saque ou depósito, e a data. 


<br />


## Rotas e descricão das funcionalidades

### * Importante:
  - Todas as requisições do tipo POST contém um middleware que valida o formato das informações requiridas no body.
  - Caso não esteja no formato experado isso irá retornar um erro.
  - Todas as rotas, com exeção da rota user para criar um novo usuário, deve conter o token no header na chave authorization;
...
- A rota recebe uma requisição POST para a criação de um novo usuario;
- É esperado da requisição as informações de email, nome do usuario e senha dentro body da requisição no seguinte formato:
    ```json
      {
...
- Após passar por essa checagem, a compra é efutada, atualizando a quantidade de ações do usuário e de corretora e 
atualizando o saldo do usuário.

- Na venda de uma ação ocorre as seguintes verificações:
  1. Se a Usuário contém a quantidade disponível para a venda.

- Após passar por essa checagem, a venda é efutada, atualizando a quantidade de ações do usuário e de corretora.
...
- Na rota de deposito e saque o body esperado na requisição do tipo POST é do seguinte formato:
- Para essas duas rotas é necessário o token de validação, pois existe um middleware que controla o acesso.
...
- Essa rota retorna o saldo da conta do usuaŕio passado pelo parametro da URL.
- Para essa rota é necessário o token de validação, pois existe um middleware que controla o acesso.
- É efetuada a verificação se a conta no parametro pertence ao usuŕio logado.
...
- Essa rota retorna a lista dos ativos que pertencem a conta passada no parametro da URL.
- Para essa rota é necessário o token de validação, pois existe um middleware que controla o acesso.
- É efetuada a verificação se a conta no parametro pertence ao usuŕio logado.
- Após a verificação o retorno da rota é do seguinte formato: 
...
- Para essa rota é necessário o token de validação, pois existe um middleware que controla o acesso.
- Essa rota só pode ser acessada por um usuario do tipo admin, existe um middleware de validação que confere qual é a categoria do usuario através do token.
